...
// Origin: Hermes 
//
// Description: Heat conduction demo program. Solves the heat equation on a 2D grid.
...
void heatBoundary(heatGrid* grid, dataMPI* mympi)
{
    ...
    MPI_Recv  (&(grid->theta[mympi->start_y+mympi->num_cells_y][mympi->start_x]),
            mympi->num_cells_x+1, MPI_DOUBLE, mympi->down, 123, mympi->cart, &status);
    ...
    MPI_Recv  (&(grid->theta[mympi->start_y-1][mympi->start_x]),
            mympi->num_cells_x+1, MPI_DOUBLE, mympi->up, 123, mympi->cart, &status);
    ...
    MPI_Recv  (&(grid->theta[mympi->start_y][mympi->start_x-1]),
            1, mympi->columntype, MPI_ANY_SOURCE, MPI_ANY_TAG, mympi->cart, &status);
