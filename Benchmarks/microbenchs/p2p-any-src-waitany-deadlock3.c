...
// Origin: Hermes
//
// Description: A deadlock occurs if P0 first send is non-blocking and it's
// second send happens before P1's.
//              buf_size should be smaller than the tmp buffer size used by send
//              implementation.
//
...
int main(int argc, char **argv) {
  ...
  if (nprocs < 3) {
    ...
    MPI_Recv(&token1, buf_size + 1, MPI_INT, 0, MPI_ANY_TAG, MPI_COMM_WORLD,
             MPI_STATUS_IGNORE);
  } else if (rank == 2) {
    MPI_Recv(&token1, buf_size + 1, MPI_INT, MPI_ANY_SOURCE, MPI_ANY_TAG,
