...
//
// Description: Many collectives called correctly
//
//// List of features
// P2P: Lacking
// iP2P: Lacking
...
int main(int argc, char** argv)
{
...
#ifdef RUN_GATHERV
  for (i = 0; i < OP_COUNT; i++)
    MPI_Gatherv(&buf0[rank * buf_size], (rank < buf_size) ? rank : buf_size, MPI_INT, buf1, rcounts, displs, MPI_INT, 0,
                comm);
#endif

#ifdef RUN_SCATTERV
  for (i = 0; i < OP_COUNT; i++)
    MPI_Scatterv(buf1, counts, displs, MPI_INT, buf0, (rank < buf_size) ? rank : buf_size, MPI_INT, 0, comm);
#endif
  ...
  for (i = 0; i < OP_COUNT; i++)
    MPI_Allgatherv(buf0, (rank < buf_size) ? rank : buf_size, MPI_INT, buf1, rcounts, displs, MPI_INT, comm);
#endif

#ifdef RUN_ALLTOALLV
  for (i = 0; i < OP_COUNT; i++)
    MPI_Alltoallv(buf1, alltoallvcounts, displs, MPI_INT, buf0, alltoallvcounts, displs, MPI_INT, comm);
#endif
