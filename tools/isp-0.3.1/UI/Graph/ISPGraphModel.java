...
//            transition.yLoc = (timeStep+1) * yGap + yRef;  
    ...
    public void addDeadlock() {        
        ...
        int nP = GlobalStructure.getInstance().noProcess;
        for(int i = 0; i < nP; i++) {
	    int row_in_process = GlobalStructure.getInstance().interleavings.get(iIndex-1).tList.get(i).size();
            nRows = nRows > row_in_process ? nRows : row_in_process;
	}
        //nRows = nRows / nP;
        ...
        if(iIndex == GlobalStructure.getInstance().nInterleavings) {
            if(GlobalStructure.getInstance().deadlocked) {                
                ...
                int yLoc = (nRows + 1) * yGap + yRef;       // allow 1 approximations 
