...
int main (int argc, char** argv)
{
    ...
    if (rank == 0)
        MPI_Sendrecv(cloud, 1, indexedtype, 1, 42, cloud + 5, 1, indexedtype, 1, 42, MPI_COMM_WORLD, &status);
    if (rank == 1)
        MPI_Sendrecv(cloud, 1, indexedtype, 0, 42, cloud + 25, 1, indexedtype, 0, 42, MPI_COMM_WORLD, &status);
